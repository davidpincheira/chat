...
export class GroupsProvider {
  ...
  addgroup(newGroup) {  //agrega un grupo 
  ...
  getmygroups() { //trae solo los grupos de un usuario
    this.firegroup.child(firebase.auth().currentUser.uid).once('value', (snapshot) => { //trae todos los grupos que un usuario creó
      this.mygroups = [];
  ...
  getgroupimage() { //obtiene la imagen del grupo
  ...
  addmember(newmember) { //agrega un miembro a un grupo ( se crea en miembro una fk de un grupo)
  ...
  deletemember(member) {           //eliminar un miembro del grupo
  ...
  getgroupmembers() {//muestra la lista de miembros al dueño del grupo
    this.firegroup.child(firebase.auth().currentUser.uid).child(this.currentgroupname).once('value', (snapshot) => {
  ...
  addgroupmsg(newmessage) { //envia un mensaje a un grupo
